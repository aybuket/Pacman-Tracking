...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(0, self.numParticles):
            self.listofPart.append(self.legalPositions[i % len(self.legalPositions)])
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance == None:
            self.listofPart = [self.getJailPosition()]
            self.beliefs = util.Counter()
        ...
        if self.beliefs.totalCount() == 0:
            ...
            self.listofPart = []
            for i in range(0, self.numParticles):
                self.listofPart.append(util.sample(self.beliefs))
    ...
    def elapseTime(self, gameState):
        ...
        """self.listofPart = []
                ...
                for p in self.legalPositions:
                    newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
                    if num > 0:
                    self.listofPart.append(util.sample(newPosDist))
                    num -= 1"""
    ...
    def getBeliefDistribution(self):
        ...
        self.beliefs = util.Counter()
        for i in range(0, self.numParticles):
            if self.beliefs.__contains__(self.listofPart[i]):
                self.beliefs[self.listofPart[i]] += 1
            else:
                self.beliefs[self.listofPart[i]] == 1
        self.beliefs.normalize()
        return self.beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for par in cartesian:
            if par[0] == par[1]:
                cartesian.remove(par)
        for i in range(0,self.numParticles):
            self.listofparticles.append(cartesian[i % len(cartesian)])
    ...
    def observeState(self, gameState):
        ...
        for j in range(0,len(self.listofparticles)):
            particle = self.listofparticles[j]
            emissionProbs = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    self.listofparticles[j] = self.getParticleWithGhostInJail(particle,i)
                else:
                    trueDistance = util.manhattanDistance(particle[i],pacmanPosition)
                    if emissionModels[i][trueDistance] > 0:
                        emissionProbs *= emissionModels[i][trueDistance]
                    else:
                        emissionProbs = 0
            beliefs[particle] += emissionProbs
        beliefs.normalize()
        if beliefs.totalCount() == 0:
            ...
            for j in range(0,len(self.listofparticles)):
                self.listofparticles[j] = util.sample(beliefs)
    ...
    def getBeliefDistribution(self):
        ...
        for i in range(0, len(self.listofparticles)):
            ...
            if beliefs.__contains__(particle):
